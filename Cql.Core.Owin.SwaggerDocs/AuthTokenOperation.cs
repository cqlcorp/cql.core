...
namespace Cql.Core.Owin.SwaggerDocs
{
    public class AuthTokenOperation : IDocumentFilter
    {
        ...
        {
            if (SupportsPasswordGrant)
            {
                AddTokenPasswordGrantEndPoint(swaggerDoc, schemaRegistry, apiExplorer);
            }

            if (SupportsRefreshTokenGrant)
            {
                AddRefreshTokenEndpoint(swaggerDoc, schemaRegistry, apiExplorer);
            }

            AddAdditionalAuthTokenEndPoints(swaggerDoc, schemaRegistry, apiExplorer);
        }
            ...
            swaggerDoc.paths.Add(
                ...
                new PathItem
                {
                    post = new Operation
                    {
                        ...
                        responses = GetTokenReponse(),
                        parameters = GetRefreshTokenGrantParameters()
                    }
                });
        }
            ...
            swaggerDoc.paths.Add(
                ...
                new PathItem
                {
                    post = new Operation
                    {
                        ...
                        consumes = FormEncodedContent(),
                        responses = GetTokenReponse(),
                        parameters = GetPasswordGrantParameters()
                    }
                });
        }
        ...
        protected virtual IDictionary<string, Response> GetTokenReponse()
        {
            return new Dictionary<string, Response>
            {
                ["200"] = new Response
                {
                    ...
                    schema = new Schema
                    {
                        properties = GetDefaultTokenReponseProperties()
                    }
                }
            };
        }
        ...
        #region Token Reponse Properties

        protected virtual IDictionary<string, Schema> GetDefaultTokenReponseProperties()
        {
        ...
        protected virtual void SetTokenProps(IDictionary<string, Schema> props)
        {
            ...
            props["expires_in"] = new Schema
            {
                example = 1799,
                type = "number",
                description = "The number of seconds before the issued access_token expires (about 30 minutes)."
            };
        ...
        {
            props["userName"] = new Schema
            {
                example = "user@cqlcorp.com",
                type = "string",
                description = "The username."
            };

            props["displayName"] = new Schema
            {
                example = "Ima User",
                type = "string",
