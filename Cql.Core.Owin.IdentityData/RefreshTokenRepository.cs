...
namespace Cql.Core.Owin.IdentityData
{
    ...
    {
        public RefreshTokenRepository(DatabaseConnection connection) : base(connection)
        {
        }
        ...
        public virtual async Task<bool> AddRefreshToken(RefreshToken token)
        {
            ...
            await Execute(db => db.ExecuteAsync("dbo.spRefreshToken_add", args, commandType: CommandType.StoredProcedure));
        ...
        public virtual Task<RefreshToken> FindRefreshToken(string refreshTokenId)
        {
            ...
            return Execute(db => db.QuerySingleOrDefaultAsync<RefreshToken>(sql, args));
        }

        public virtual async Task<IList<RefreshToken>> GetAllRefreshTokens()
        {
            var q = await Execute(db => db.QueryAsync<RefreshToken>("SELECT * FROM dbo.RefreshTokens"));
        ...
        public virtual async Task<bool> RemoveRefreshToken(string refreshTokenId)
        {
            ...
            return await Execute(db => db.ExecuteAsync(sql, args)) > 0;
        }

        public virtual Task<bool> RemoveRefreshToken(RefreshToken refreshToken)
        {
            return RemoveRefreshToken(refreshToken.Id);
        }
    }
}
