...
namespace Cql.Core.Owin.IdentityData
{
    ...
    {
        public UserLoginRepository(DatabaseConnection connection) : base(connection)
        {
        }
...
";
        ...
        public virtual Task<IdentityUser> FindAsync(UserLoginInfo login)
        {
            ...
            const string sql = "SELECT u.* " +
                               ...
                               " JOIN dbo.AspNetUserLogins ul ON ul.UserId = u.Id " +
        ...
        {
            const string sql =
                "SELECT u.LoginProvider, u.ProviderKey " +
                "FROM dbo.AspNetUserLogins u " +
                "WHERE u.UserId = @0";

            var q = await Execute(db => db.QueryAsync<UserLoginRecord>(sql, user.Id));
