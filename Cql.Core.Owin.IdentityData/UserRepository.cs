namespace Cql.Core.Owin.IdentityData
{
    ...
    public class UserRepository : RepositoryBase, IUserRepository
    {
        ...
        public virtual IQueryable<IdentityUser> Users => throw new NotImplementedException();

        public virtual async Task CreateAsync(IdentityUser user)
        {
            var args = GetArgsFromUser(user);

            const string sql = "INSERT INTO dbo.AspNetUsers (" + " Email, EmailConfirmed, PasswordHash, SecurityStamp, PhoneNumber,"
                               + " PhoneNumberConfirmed, TwoFactorEnabled, LockoutEndDate, LockoutEnabled, AccessFailedCount, UserName) " + "OUTPUT INSERTED.* " + "VALUES ("
            ...
            var createdUser = await this.Execute(db => db.QuerySingleAsync<IdentityUser>(sql, args));
        ...
        public virtual Task<IdentityUser> FindByEmailAsync(string email)
        {
            ...
            const string sql = "SELECT TOP 1 * " + "FROM dbo.AspNetUsers u " + "WHERE u.Email LIKE @Email " + "ORDER BY u.UserName";
        ...
        {
            var args = new { @Guid = guid };

            const string sql = "SELECT * " + "FROM dbo.AspNetUsers " + "WHERE Guid = @Guid";

            return this.Execute(db => db.QuerySingleOrDefaultAsync<IdentityUser>(sql, args));
        }
        ...
        public virtual Task<IdentityUser> FindByNameAsync(string userName)
        {
            ...
            const string sql = "SELECT * " + "FROM dbo.AspNetUsers " + "WHERE UserName = @UserName";
        ...
        public virtual Task UpdateAsync(IdentityUser user)
        {
            ...
            const string sql = "UPDATE dbo.AspNetUsers " + "SET " + " Email = @Email," + " EmailConfirmed = @EmailConfirmed," + " PasswordHash = @PasswordHash,"
                               + " SecurityStamp = @SecurityStamp," + " PhoneNumber = @PhoneNumber," + " PhoneNumberConfirmed = @PhoneNumberConfirmed,"
                               + " TwoFactorEnabled = @TwoFactorEnabled," + " LockoutEndDate = @LockoutEndDate," + " LockoutEnabled = @LockoutEnabled,"
