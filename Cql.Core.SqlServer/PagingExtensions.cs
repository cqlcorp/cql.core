...
namespace Cql.Core.SqlServer
{
    public static class PagingExtensions
    {
        ...
        /// </summary>
        public static void AddPagingAndSortFilters<TSortBy>(this DynamicParameters args, SearchFilter<TSortBy> filter) where TSortBy : struct
        {
        ...
        {
            return await db.CountAsync<T>(s =>
            {
        ...
        {
            var records = await db.FindAsync<T>(s =>
            {
                if (filter.WhereClause != null)
                {
                    s.Where(filter.WhereClause);
                ...
                s.OrderBy(filter.OrderByClause)
                    .Skip(filter.GetSkip())
                    .Top(filter.GetTake())
                    .WithParameters(filter.Parameters);
            });
